...
static short        gPipeProcessedBuf[AUDIO_PERIOD_FRAMES * AUDIO_CHANNELS];

static int pipeCapCallback(short *pcmData, int frameCount, void *userData) {
    (void)userData;

    if (frameCount > AUDIO_PERIOD_FRAMES) {
        fprintf(stderr, "[Pipe] frameCount too large: %d\n", frameCount);
        return -1;
    }

    /* 录音产出第一帧后再初始化播放，配合播放端静音预填充。 */
    if (!gPipePbReady) {
    ...
    dspPassThrough(pcmData, gPipeProcessedBuf, frameCount);
    ...
    /* 推到播放 */
    if (gPipePbReady) {
        audioPbWrite(gPipeProcessedBuf, frameCount);
    }
