...
static int skipChunkBytes(FILE *fp, unsigned int chunkSize) {
    long skipSize = (long)(chunkSize + (chunkSize & 1U));
    return fseek(fp, skipSize, SEEK_CUR);
}
...
int audioWavOpenRead(audioWavFile *wav, const char *fileName) {
    ...
    if (fread(chunkId, 1, 4, wav->fp) != 4 || strncmp(chunkId, "RIFF", 4) != 0) {
        fclose(wav->fp); wav->fp = NULL; return -1;
    }
    ...
    while (fread(chunkId, 1, 4, wav->fp) == 4) {
        ...
        if (strncmp(chunkId, "fmt ", 4) == 0) {
            ...
            /* 跳过 fmt chunk 剩余字节 */
            if ((chunkSize > 16) &&
                    (skipChunkBytes(wav->fp, chunkSize - 16) != 0)) {
                fclose(wav->fp); wav->fp = NULL; return -1;
            }
            ...
            /* 跳过未知 chunk */
            if (skipChunkBytes(wav->fp, chunkSize) != 0) {
                fclose(wav->fp); wav->fp = NULL; return -1;
            }
        }
    }
    ...
    if ((wav->dataSize <= 0) || (wav->channels <= 0) ||
            (wav->sampleRate <= 0) || (wav->bitDepth <= 0) ||
            ((wav->bitDepth % 8) != 0) || (wav->blockAlign <= 0) ||
            (wav->byteRate <= 0)) {
        fclose(wav->fp); wav->fp = NULL; return -1;
    }

    expectedBlockAlign = wav->channels * (wav->bitDepth / 8);
    expectedByteRate = wav->sampleRate * expectedBlockAlign;
    if ((wav->blockAlign != expectedBlockAlign) ||
            (wav->byteRate != expectedByteRate) ||
